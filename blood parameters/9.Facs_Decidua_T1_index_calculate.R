...
##reading FACS index
FACS_index<-as.data.frame(read_excel("D:/PROJECT/新冠/manuscript/1.data/4.Facs_data/流式各群占比计算.xlsx", sheet =6, col_names = T, col_types = NULL, na = "", skip = 0))
head(FACS_index)
##B91的流式结果存在异常,但仅去除未固定的A60和A61
FACS_index<-FACS_index[which(!(FACS_index$sample %in% c("A60","A61_no_fix"))),]
data_anlysis<-merge(clinical_data,FACS_index,by="sample")
head(data_anlysis);dim(FACS_index);dim(data_anlysis)#10 37 41
...
data_anlysis$Index_1<-round((data_anlysis$P5/data_anlysis$P4)*100,3)
data_anlysis$Index_2<-round((data_anlysis$CD8/data_anlysis$P5)*100,3)
data_anlysis$Index_3<-round((data_anlysis$CD4/data_anlysis$P5)*100,3)
...
write.table(as.data.frame(data_anlysis), file="D:/PROJECT/新冠/manuscript/3.Table/Facs/Facs_Decidua_T1_index_all_sample_infection_data.txt",sep="\t",quote=F, row.names=F, col.names=T) 
##数据准备
head(data_anlysis)
...
Facs_index_boxplot<-ggboxplot(analysis_final2,x="Infect_group", y="ratio", add = "jitter",size =0.5,fill="Infect_group") +  
  ...
  scale_fill_manual(values=c("navy","red"))+ylab("% ratio")+labs(title = "Decidua_T1_index")+
...
Facs_index_boxplot
ggsave("D:/PROJECT/新冠/manuscript/2.Figure/Facs/plot_figure/Decidua/all_sample/Facs_Decidua_T1_index_all_sample_compare_stat_box_infection_data_split.pdf",Facs_index_boxplot,width=24, height=26)
##绘制相关性图
...
index_plot<-ggplot(analysis_final2, aes(x = Infect_day, y = ratio)) + #color = group,
  xlab("infect day to last menstrual period")+ylab("Cell ratio(%)") +labs(title = "Decidua_T1_index")+
...
ggsave(index_plot1,file="D:/PROJECT/新冠/manuscript/2.Figure/Facs/plot_figure/Decidua/all_sample/Facs_Decidua_T1_index_Dot_smooth_plot_for_all_sample_infection_data_split.pdf",width = 24, height =26)
ggsave(index_plot2,file="D:/PROJECT/新冠/manuscript/2.Figure/Facs/plot_figure/Decidua/all_sample/Facs_Decidua_T1_index_Dot_line_plot_for_all_sample_infection_data_split.pdf",width = 24, height =26)
ggsave(index_plot3,file="D:/PROJECT/新冠/manuscript/2.Figure/Facs/plot_figure/Decidua/all_sample/Facs_Decidua_T1_index_Dot_lm_plot_for_all_sample_infection_data_split.pdf",width = 24, height =26)
ggsave(index_plot4,file="D:/PROJECT/新冠/manuscript/2.Figure/Facs/plot_figure/Decidua/all_sample/Facs_Decidua_T1_index_Dot_lm_plot_for_all_sample_infection_data_split2.pdf",width = 24, height =26)
...
Facs_index_boxplot2<-ggboxplot(analysis_final3,x="Infect_group", y="ratio", add = "jitter",size =0.5,fill="Infect_group") +  
  ...
  theme(axis.text.x = element_text(size = 9,colour = 'black',vjust=0.5,hjust=0.5,angle = 0),
        legend.title = element_text(size = 9))+ facet_wrap(~ Details, scales = "free",ncol =6)
Facs_index_boxplot2
ggsave("D:/PROJECT/新冠/manuscript/2.Figure/Facs/plot_figure/Decidua/P5_remove/Facs_Decidua_T1_index_P5_remove_sample_compare_stat_box_infection_data_split.pdf",Facs_index_boxplot2,width=24, height=26)

##绘制相关性图
range(analysis_final3$Infect_day)#33 89
range(analysis_final3$Pregnant_day)#42 70
##输出相关性统计结果
write.table(as.data.frame(analysis_final3[,c("Details","patient","Infect_day","ratio")]), file="D:/PROJECT/新冠/manuscript/3.Table/Facs/Cell_ratio_Decidua_T1_index_for_P5_remove_sample_infection_data.txt",sep = "\t",row.names=F) 
...
write.table(as.data.frame(correlation_results), file="D:/PROJECT/新冠/manuscript/3.Table/Facs/lm_spearman_Decidua_T1_index_for_P5_remove_sample_infection_data.txt",sep = "\t",row.names=F) 
...
#`geom_smooth()` using method = 'loess' and formula 'y ~ x'
ggsave(index_plot10,file="D:/PROJECT/新冠/manuscript/2.Figure/Facs/plot_figure/Decidua/P5_remove/Facs_Decidua_T1_index_Dot_smooth_plot_for_P5_remove_sample_infection_data_split.pdf",width = 24, height =26)
ggsave(index_plot20,file="D:/PROJECT/新冠/manuscript/2.Figure/Facs/plot_figure/Decidua/P5_remove/Facs_Decidua_T1_index_Dot_line_plot_for_P5_remove_sample_infection_data_split.pdf",width = 24, height =26)
ggsave(index_plot30,file="D:/PROJECT/新冠/manuscript/2.Figure/Facs/plot_figure/Decidua/P5_remove/Facs_Decidua_T1_index_Dot_lm_plot_for_P5_remove_sample_infection_data_split.pdf",width = 24, height =26)
ggsave(index_plot40,file="D:/PROJECT/新冠/manuscript/2.Figure/Facs/plot_figure/Decidua/P5_remove/Facs_Decidua_T1_index_Dot_lm_plot_for_P5_remove_sample_infection_data_split2.pdf",width = 24, height =26)
